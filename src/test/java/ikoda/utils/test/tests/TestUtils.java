...
public class TestUtils
{
	...
	private void testMLSErverStreamOpen() throws Exception
	{
		try
		{
			Spreadsheet.getInstance().initCsvSpreadsheet("CATEGORY_LOG", "C:\\Users\\jake\\__workspace\\ikoda-utils\\src\\test\\resources");
			...
			String response=Spreadsheet.getInstance().getCsvSpreadSheet("CATEGORY_LOG").sparkStreamInit("192.168.0.141", 9999);
	...
	private void testMLServerStream() throws Exception
	{
		try
		{
			...
			String response=Spreadsheet.getInstance().getCsvSpreadSheet("CATEGORY_LOG").sparkStreamRun("192.168.0.141", 9999);
	...
	private void testMLServerStreamLibSvm() throws Exception
	{
		...
		{
			logger.info("testMLServerStream");
			Spreadsheet.getInstance().initLibsvm2("test", "TargetColumn",  "C:\\Users\\jake\\__workspace\\ikoda-utils\\src\\test\\resources");
			...
			logger.info("\n\n\n\n\nLoaded Rows"+Spreadsheet.getInstance().getLibSvmProcessor("test").rowCount()+" Cols:"+Spreadsheet.getInstance().getLibSvmProcessor("test").columnCount()+"\n\n\n");
			...
			String response1=Spreadsheet.getInstance().getLibSvmProcessor("test").sparkStreamInit("192.168.0.141", 9999);
			String response=Spreadsheet.getInstance().getLibSvmProcessor("test").sparkStreamRunLibsvm("192.168.0.141", 9999);
			assertTrue(response.toUpperCase().equals("SUCCESS"));
		}
		catch(Exception e)
		{
	...
	private void testMergeLibSvm() throws Exception
	{
		try
		{
			...
			Spreadsheet.getInstance().initLibsvm2("_part0", "A_AggregatedMajor",  "C:\\Users\\jake\\__workspace\\ikoda-utils\\src\\test\\resources");
			...
			Spreadsheet.getInstance().initLibsvm2("_part1", "A_AggregatedMajor",  "C:\\Users\\jake\\__workspace\\ikoda-utils\\src\\test\\resources");
			...
			Spreadsheet.getInstance().getLibSvmProcessor("_part0").mergeIntoLibsvm(Spreadsheet.getInstance().getLibSvmProcessor("_part1"));
	...
	private void testMergeLibSvmValidated() throws Exception
	{
		try
		{
			logger.info("testMLServerStream");
			String[]  s = {};
			Spreadsheet.getInstance().initLibsvm2("_part00", "A_AggregatedMajor",  "C:\\Users\\jake\\__workspace\\ikoda-utils\\src\\test\\resources");
			...
			Spreadsheet.getInstance().getLibSvmProcessor("_part1").setPkColumnName("A_UID");
			Spreadsheet.getInstance().getLibSvmProcessor("_part1").loadLibsvm("_part1");
			Spreadsheet.getInstance().getLibSvmProcessor("_part00").mergeIntoLibsvmValidating(Spreadsheet.getInstance().getLibSvmProcessor("_part1"));
